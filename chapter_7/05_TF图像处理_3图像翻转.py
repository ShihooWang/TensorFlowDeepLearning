...
with tf.Session() as sess:
    ...
    # filpped = tf.image.flip_up_down(img_data)  # 上下翻转
    # filpped = tf.image.flip_left_right(img_data)  # 左右翻转
    filpped = tf.image.transpose_image(img_data)  # 沿对角线翻转
    # filpped = tf.image.random_flip_left_right(img_data)  # 50%概率左右翻转
    # filpped = tf.image.random_flip_up_down(img_data)  # 50%概率随机上下翻转
